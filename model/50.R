comment="glmnet on 1-3-gram,bintf,calibrated with NB(no prior),no standardize"
require(Metrics)
require(glmnet)
...
apply.model <- function(model,X){
  ...
  pred <- apply.multi.NB.normal(model$nb,pred,add.prior=FALSE)
}
train.model <- function(X,y){
  ...
  kappa <- sapply(1:K,function(k){
    ...
    pred <- apply.multi.NB.normal(nb,pred,add.prior=FALSE)
