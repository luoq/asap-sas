comment="glmnet on simple features and 1-3-gram,bintf,calibration with NB"
require(Metrics)
require(glmnet)
require(e1071)
...
apply.model <- function(model,sf,dtm){
  ...
  pred <- factor2numeric(predict(model$nb,pred))
}
train.model <- function(sf,dtm,y){
  ...
  nb <- naiveBayes(predict(fit,X,s=s),as.factor(y))
