comment="glmnet on 1-3-gram,bintf,calibrated with NB(KDE,no prior),no standardize"
require(Metrics)
require(glmnet)
...
train.model <- function(X,y){
  ...
  kappa <- sapply(1:K,function(k){
    ...
    nb <- train.multi.NB.KDE(predict(fit,X[-omit,,drop=FALSE]),y[-omit])
    pred <- predict(fit,X[omit,,drop=FALSE])
    pred <- apply.multi.NB.KDE(nb,pred,add.prior=FALSE)
  ...
  nb <- train.multi.NB.KDE(predict(fit,X,s=s),y)
...
apply.model <- function(model,X){
  ...
  pred <- apply.multi.NB.KDE(model$nb,pred,add.prior=FALSE)
}
