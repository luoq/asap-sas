comment="glmnet,multinomial on 1-3-gram,bintf,standardize=FALSE"
require(Metrics)
require(glmnet)
...
apply.model <- function(model,X)
  as.numeric(predict(model$fit,X,s=model$s,type="class"))
train.model <- function(X,y){
  K <- 5
  n <- length(y)
  all.folds <- split(1:n,rep(1:K,length=n))

  fit <- glmnet(X,as.factor(y),alpha=0.8,nlambda=100,family="multinomial",standardize=FALSE)
  lambda <- fit$lambda
  kappa <- sapply(1:K,function(k){
    omit <- all.folds[[k]]
    fit <- glmnet(X[-omit,,drop=FALSE],as.factor(y[-omit]),lambda=lambda,alpha=0.8,family="multinomial",standardize=FALSE)
    pred <- matrix(as.numeric(predict(fit,X[omit,,drop=FALSE],type="class")),nrow=length(omit))
