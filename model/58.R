comment="glmnet on 1-3-gram,bintf,calibrated with NB,no standardize,square score"
require(Metrics)
require(glmnet)
...
train.model <- function(X,y){
  ...
  fit <- glmnet(X,y^2,alpha=0.8,nlambda=100,standardize=FALSE,family="gaussian")
  lambda <- fit$lambda
  kappa <- sapply(1:K,function(k){
    omit <- all.folds[[k]]
    fit <- glmnet(X[-omit,,drop=FALSE],y[-omit]^2,lambda=lambda,alpha=0.8,,standardize=FALSE,family="gaussian")
