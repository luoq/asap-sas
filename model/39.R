comment="glmnet on 1-3-gram with bintf,calibrated with NB and some simple feature"
require(Metrics)
require(glmnet)
...
train.model <- function(sf,dtm,y){
  simple.features=c("Nword","Nchar","Nsent","DiffWord","SentLen")
  sf <- as.matrix(sf[,simple.features])
  K <- 5
  n <- length(y)
  ...
  model <- list(simple.features=simple.features,fit=fit,s=s,dist=dist)
  return(list(model=model,kappa=kappa))
}
apply.model <- function(model,sf,dtm){
  sf <- as.matrix(sf[,model$simple.features])
