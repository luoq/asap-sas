comment="NB.Bernoulli on 1-3-gram with feature selection based on IG,laplace=1e-3"
require(Metrics)
...
train.model <- function(X,y){
  ...
  kappa <- sapply(1:K,function(k){
    omit <- all.folds[[k]]
    w <- informationGain2(y[-omit],X[-omit,,drop=FALSE],laplace=LAPLACE)
