comment="glmnet on sf,calibrated with NB"
require(Metrics)
require(glmnet)
...
train.model <- function(sf,y){
  ...
  fit <- glmnet(X,y,alpha=0.8,nlambda=100,standardize=TRUE,family="gaussian")
  lambda <- fit$lambda
  kappa <- sapply(1:K,function(k){
    omit <- all.folds[[k]]
    fit <- glmnet(X[-omit,,drop=FALSE],y[-omit],lambda=lambda,alpha=0.8,,standardize=TRUE,family="gaussian")
