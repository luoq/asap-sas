comment="C-svc on 1-gram,logtf with degree 3 polynomial kernel"
require(Metrics)
require(e1071)
...
dtm_features_ctrl <- list(mingram=1,maxgram=1,local_weight="logtf",term_weight=NULL)
...
train.model <- function(X,y){
  ...
  estimate.kappa <- function(c){
    kappa <- sapply(1:K,function(k){
      omit <- all.folds[[k]]
      fit <- try(svm(X[-omit,,drop=FALSE],y[-omit],scale=FALSE,type="C-classification",kernel="polynomial",cost=c,gamma=1,coef0=0,degree=3),silent=TRUE)
  ...
  fit <- svm(X,y,scale=FALSE,type="C-classification",kernel="polynomial",cost=c,gamma=1,coef0=0,degree=3)
