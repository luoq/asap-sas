comment="bagging NB.Multinomial with binary information gain"
require(Metrics)
...
train.model <- function(X,y)
  Bagging(X,y,function(X,y)
          train.split.NB.Multinomial(X,y,split.ctrl=list(train.ratio=0.5,split="random",max.measure="kappa"),
                                     nb.ctrl=list(weight.fun=function(y,X) informationGain2(y,X!=0,laplace=LAPLACE),laplace=LAPLACE,criterion="max.probability")),
